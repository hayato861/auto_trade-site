...
function render(){
 ...
 document.querySelector('#stocks').innerHTML=visible.map(s=>`<article class="card"><div class="card-head"><div><div class="ticker">${escapeHtml(s.ticker)}</div><h3>${escapeHtml(s.name)}</h3></div><span class="badge ${escapeHtml(s.signal)}">${labels[s.signal]||'取得不可'}</span></div><div class="price">${yen(s.close)}<small>円</small></div><div class="date">${s.date?escapeHtml(s.date)+' 時点の終値':'今回の価格データを取得できませんでした'}</div><div class="averages"><div>短期 SMA ${s.pfast}日<b>${yen(s.fast)} 円</b></div><div>長期 SMA ${s.pslow}日<b>${yen(s.slow)} 円</b></div></div><details><summary>設定・100株の目安を見る</summary><dl><dt>100株の概算額</dt><dd>${yen(s.close*100)} 円</dd><dt>損切り設定</dt><dd>−${Math.round(s.sl*100)}%</dd><dt>利確設定</dt><dd>+${Math.round(s.tp*100)}%</dd><dt>終値基準の損切り</dt><dd>${yen(s.close*(1-s.sl))} 円</dd><dt>終値基準の利確</dt><dd>${yen(s.close*(1+s.tp))} 円</dd></dl></details></article>`).join('');
}
