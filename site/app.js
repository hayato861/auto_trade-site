...
const riskSettings=s=>s.rule_version?`<dt>初期の売り目安</dt><dd>購入価格から−8%</dd><dt>追従する売り条件</dt><dd>保有中の最高終値から−8%</dd><dt>固定利確</dt><dd>なし</dd><dt>判定理由</dt><dd>${escapeHtml(s.signal_reason||'変化なし')}</dd><dt>仮想保有</dt><dd>${yen(s.paper_shares||0)} 株</dd><dt>仮想購入価格</dt><dd>${yen(s.paper_entry)} 円</dd><dt>仮想売買開始</dt><dd>${escapeHtml(s.paper_start_date||'—')}</dd>`:`<dt>損切り設定</dt><dd>−${Math.round(s.sl*100)}%</dd><dt>利確設定</dt><dd>+${Math.round(s.tp*100)}%</dd><dt>終値基準の損切り</dt><dd>${yen(s.close*(1-s.sl))} 円</dd><dt>終値基準の利確</dt><dd>${yen(s.close*(1+s.tp))} 円</dd>`;
...
function render(){
 ...
 document.querySelector('#stocks').innerHTML=visible.map(s=>`<article class="card ${s.signal==='buy'?'signal-buy':s.signal==='sell'?'signal-sell':''}"><div class="card-head"><div><div class="ticker">${escapeHtml(s.ticker)}</div><h3>${escapeHtml(s.name)}</h3></div><span class="badge ${escapeHtml(s.signal)}">${labels[s.signal]||'取得不可'}</span></div><div class="stock-tools"><span>${escapeHtml(s.sector||'その他')}</span><button type="button" class="favorite" data-ticker="${escapeHtml(s.ticker)}" aria-pressed="${favorites.has(s.ticker)}" aria-label="${escapeHtml(s.name)}をお気に入り${favorites.has(s.ticker)?'から解除':'に登録'}">${favorites.has(s.ticker)?'★ 登録済み':'☆ お気に入り'}</button></div><div class="price">${yen(s.close)}<small>円</small></div>${s.data_issue?`<p class="setting-note">${escapeHtml(s.data_issue)}</p>`:''}<div class="date">${s.date?escapeHtml(s.date)+' 時点の終値':'今回の価格データを取得できませんでした'}</div><div class="averages"><div>短期 SMA ${s.pfast}日<b>${yen(s.fast)} 円</b></div><div>長期 SMA ${s.pslow}日<b>${yen(s.slow)} 円</b></div></div><details><summary>設定・100株の目安を見る</summary><p class="setting-note">${escapeHtml(s.setting_note||'既存設定')}</p><dl><dt>100株の概算額</dt><dd>${yen(s.close*100)} 円</dd>${riskSettings(s)}</dl></details>${historyPanel(s)}<a class="finance-link" href="https://finance.yahoo.co.jp/quote/${encodeURIComponent(s.ticker)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(s.name)}のYahoo!ファイナンスを開く（別タブ）">Yahoo!ファイナンスで見る <span aria-hidden="true">↗</span></a></article>`).join('');
}
